module.exports = () => {
  ...
  const personasController =
    require("../../controllers/Personas/Personas.controller")();
  const FilesUpload = require("../../middlewares/FilesUpload");
  router.post(
    "/personas",
    FilesUpload.uploadSingle(
      process.env.PATH_USERS_FOLDER_PROFILES_IMAGES,
      "fotoPerfil"
    ),
    personasController.insertOne
  );
  ...
  router.post(
    ...
    personasController.updateAvatar
  );
  ...
  router.get(
    ...
    personasController.findAllInOkNotPastores
  );
  router.get(
  ...
  router.get(
    ...
    personasController.findPhoneNumber
  );
  router.get(
    "/personas/document/:documentNumber",
    personasController.findDocumentNumber
  );
